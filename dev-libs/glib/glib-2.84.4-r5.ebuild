...
multilib_src_configure() {
	...
	if _need_bootstrap_gi ; then
		...
		if  tc-is-cross-compiler ; then
			...
			export PATH="${INTROSPECTION_CHOST_PREFIX}/usr/bin:${PATH}"
		fi
		...
		# share/pkgconfig), since bug #951487-style checks above only guarantee
		# *a* gobject-introspection-1.0.pc is visible, not that every other
		# dependency .pc (e.g. libpcre2-8) stays reachable once we override
		# PKG_CONFIG_LIBDIR for the rest of this configure run.
		local -x PKG_CONFIG_LIBDIR="${INTROSPECTION_LIB_DIR}/pkgconfig:${INTROSPECTION_BUILD_DIR}/meson-private:${SYSROOT}/usr/$(get_libdir)/pkgconfig:${SYSROOT}/usr/share/pkgconfig"
		# NOTE: PKG_CONFIG_PATH must stay non-empty here, not just unset-safe:
		# some code further down the meson/cross-pkg-config stack substitutes
		# an empty PKG_CONFIG_PATH with a host default (/usr/share/pkgconfig),
		# which then shadows our CTARGET dirs. Keep it mirroring LIBDIR so it's
