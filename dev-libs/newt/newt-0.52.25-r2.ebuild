...
src_prepare() {
	sed -i Makefile.in \
		...
		-e 's|	make |	$(MAKE) |g' \
		-e "s|	ar |	$(tc-getAR) |g" \
		-e 's|ifneq ($(GNU_LD),)|ifeq (1,1)|g' \
		|| die "sed Makefile.in"

# Вибиваємо дурну перевірку на GNU_LD, щоб SHLIBFLAGS зшивався завжди
    sed -i 's/ifneq ($(GNU_LD),)/ifneq (1,)/g' Makefile.in || die
    # Або просто захардкодити прапори, прибравши else:
    # sed -i '/else/,/endif/d' Makefile.in || die
