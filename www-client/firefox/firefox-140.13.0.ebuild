...
src_prepare() {
	...
	eapply "${FILESDIR}/0001-libcxx-21-fix.patch"
	...
	# Do not force RUST_TARGET while cross-compiling: Firefox configure uses
	# this env var for both host and target rust triplet detection.
	if ! use elibc_glibc && [[ ${CBUILD} == ${CHOST} ]] ; then
		if use amd64 ; then
...
src_configure() {
	...
	# Rust target resolution in Firefox's configure cannot always map
